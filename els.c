...
bit Check(void){ //冲突检测
	bit s;
	if(((form[number][dir][3]<<(12-form_x)) | BackGround[form_y+4]) == ((form[number][dir][3]<<(12-form_x))+BackGround[form_y+4])){
		if(((form[number][dir][2]<<(12-form_x)) | BackGround[form_y+3]) == ((form[number][dir][2]<<(12-form_x))+BackGround[form_y+3])){
			if(((form[number][dir][1]<<(12-form_x)) | BackGround[form_y+2]) == ((form[number][dir][1]<<(12-form_x))+BackGround[form_y+2]))
				s=1;																						  
			}
		}
	else 
...
void Add2Background(void){
	u8 i;
	for(i=0;i<4;i++){
		BackGround[form_y+i] += (form[number][dir][i]<<(12-form_x)); 
	   }
